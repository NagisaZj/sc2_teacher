            ...
            while True:
                ...
                if total_step % UPDATE_GLOBAL_ITER == 0 or done:
                    ...
                    buffer_s, buffer_a0, buffer_a1, buffer_a2, buffer_v_target, buffer_avail, buffer_a0_exp, buffer_a1_exp, buffer_a2_exp = np.vstack(
                        buffer_s), np.vstack(buffer_a0), np.vstack(buffer_a1), np.vstack(
                        buffer_a2), np.vstack(buffer_v_target), np.vstack(
                        buffer_avail), np.vstack(buffer_a0_exp), np.vstack(buffer_a1_exp), np.vstack(
                        buffer_a2_exp)  # put together into a single array
                    feed_dict = {
                        self.AC.s: buffer_s,
                        self.AC.a0: buffer_a0,
                        self.AC.a1: buffer_a1,
                    ...
                    entropy, aloss, td, exp_loss = sess.run(
                        [self.AC.entropy, self.AC.a_loss, self.AC.td, self.AC.exp_loss], feed_dict=feed_dict)
