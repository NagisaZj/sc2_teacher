...
flags.DEFINE_string("save_dir", "./model", "directory for saving")
...
class Input:
    ...
    def train_feed(self,state, action, reward, next_state, done):
        return {
            self.state:state,
            self.action:action,
            self.reward:reward,
...
class Train :
    def __init__(self, model, name):
        with tf.variable_scope(name) as scope:
            #select the max q action
            max_t_q_map = tf.reduce_max(model.t_q_map,axis=[1,2,3])
            max_t_q_other = tf.reduce_max(model.t_q_other, axis=1)
            max_t_q = tf.reduce_max((max_t_q_map,max_t_q_other),axis=0)

            target = model.input.reward + (1.0 - model.input.done) * model.gamma * max_t_q
...
class Memory:
    ...
    def sample(self, batch_size):
        indices = np.random.permutation(len(self.mem))[:batch_size]
    ...
    def enqueue(self, state, action, reward, next_state, done):
...
def main():
    ...
    while step < FLAGS.number_steps:
        ...
        if FLAGS.test or (step < FLAGS.training_start) or (step % FLAGS.learn_freq != 0):
        ...
        print(q_other.shape)
        print('q_max: %f, q_min: %f, q_noop: %f, q_select: %f, action: %d, reward: %f' % (max(q_map.max(), q_other.max()), min(q_map.min(), q_other.min()), q_other[0][0], q_other[0][1], action, reward))
